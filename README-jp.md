...
- 変数とは。メモリ領域へ付けた名札(label, ラベル)である
- 変数の値とは、変数で指定されるメモリ領域に保存されている値を読み出す操作である
- 変数に値を割り当てるとは、値を変数で指定されるメモリ領域へ書き込む操作である

この仕組みは破壊的代入(destructive assignment)と呼ばれています。

この破壊的代入を不変オブジェクト指向(immutable object oriented)パラダイムの元で実装したものが
リファンスオブジェクトであり、以下のメッセージに対して応答します。

- &Ref.make : Top -> Ref    -- 引数を初期値とするメモリ領域を１つ確保し、それをリファレンスで返す
- Ref#peek! : Top           -- リファレンスが持つメモリ領域に保存されている値を読み出し、その値を返す
- Ref#poke! : Top -> Unit   -- 引数の値をリファレンスが持つメモリ領域へ書き込む

```
umu:1> val rx = &Ref.make 3        # 3 を初期値とするリファレンスを返す
val rx : Ref = #Ref<3>
umu:2> rx.peek!                    # リファレンスから現在の値を読み出す ==> 3
val it : Int = 3
umu:3> rx.poke! <| rx.peek!.+ 4    # その現在の値に 4 を足し、その結果をリファレンスへ書き込む
val it : Unit = ()
...
単純な仕組みですが、冗長です。
そこで、これらの操作を抽象化した標準関数 `ref` および標準演算子 `!!` と `:=` を提供します。

```
umu:1> val rx = ref 3                # 3 を初期値とするリファレンスを返す
val rx : Ref = #Ref<3>
umu:2> !!rx                          # リファレンスから現在の値を読み出す ==> 3
val it : Int = 3
umu:3> rx := !!rx + 4                # その現在の値に 4 を足し、その結果をリファレンスへ書き込む
val it : Unit = ()
